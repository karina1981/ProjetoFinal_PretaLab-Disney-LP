...
function calcularMedia() {
  ...
  if (result === 0 ) {
  ...
  else if (result >= 0.1 && result <= 3) {
    mensagem = `Caramba, deu ruim, você obteve media ${result}! Estude mais e tente novamente!`
  }

  else if (result >= 3.1 && result <= 5.9) {
    mensagem = `Você obteve media ${result}! Falta pouco para a média.`
  }

  else if (result >= 6 && result <= 7) {
    mensagem = `Você está na média com ${result}`
  }

  else if (result >= 7.1 && result <= 9.9) {
    mensagem = `Notão! Sua média é ${result}!`
  }
    ...
    mensagem = `Não pode existir nota maior que 10! e menor que 0!`
  }
